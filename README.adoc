...
* Install the psql password file for this account:
	...
	cp configs/pgpass.conf ~/.pgpass   # *Nix, Mac
* Create databases:
	psql <<!
	...
	!
* Decide if you want JMS or not. If so, use standalone-full.xml in the following; if not, use standalone.xml
and delete sourcecode/ex53solution
* Merge configs/datasources into wildfly config file.
