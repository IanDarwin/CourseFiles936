...
. 1 is the normal deployment message
. 2 Several debug printlns from CommentReceiver (format varies as you wrote this) dumping the form
. 3 JPA Provider (Hibernate) getting ready to insert
. 4 Output from CommentReceiver after giving data to JPA 
. 5 JPA Provider trace of the insert
. 6 This is from the second "send" that you are asked to do in CommentSender
. 7 We closed the Session, so the other end gets notified and cleans up.

Note that becuase of threading, the lines can appear in almost any order.
